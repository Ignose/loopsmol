import { availableAmount, buy, chew, cliExecute, drink, eat, equip, familiarEquippedEquipment, getIngredients, haveEffect, Item, itemAmount, itemType, mallPrice, myAdventures, myBasestat, myDaycount, myFullness, myInebriety, myPrimestat, mySpleenUse, print, restoreMp, retrieveItem, reverseNumberology, setProperty, toInt, turnsPerCast, use, useFamiliar, useSkill } from "kolmafia";
import { Quest } from "../engine/task";
import { args } from "../args";
import { $effect, $familiar, $item, $items, $skill, $slot, clamp, Diet, get, getAverageAdventures, getRemainingLiver, getRemainingSpleen, getRemainingStomach, have, MenuItem, sumNumbers } from "libram";
  ...
  tasks: [
    {
      ...
      after: [],
      completed: () =>
        myDaycount() > 1 || (myFullness() >= args.minor.stomach && myInebriety() >= args.minor.liver),
      ready: () => myBasestat(myPrimestat()) >= 149 || myAdventures() <= 1,
      do: (): void => {
        if (have($item`astral six-pack`)) {
          use($item`astral six-pack`);
        }
        const MPA = args.minor.voa;
        ...
        const food = Math.max(args.minor.stomach - myFullness(), 0);
        const booze = Math.max(args.minor.liver - myInebriety(), 0);
        const spleen = Math.max(args.minor.spleen - mySpleenUse(), 0);
